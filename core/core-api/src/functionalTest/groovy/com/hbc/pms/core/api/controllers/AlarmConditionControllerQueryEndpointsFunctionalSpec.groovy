...
class AlarmConditionControllerQueryEndpointsFunctionalSpec extends FunctionalTestSpec {
  ...
  def "Get all alarm conditions - OK"() {
    when:
    def response
            = restClient.get("${ALARM_CONDITION_PATH}", ApiResponse<List<AlarmConditionResponse>>)
  ...
  def "Get alarm condition by Id - OK"() {
    when:
    def response
            = restClient.get("${ALARM_CONDITION_PATH}/$DEFAULT_CONDITION_ID", ApiResponse<AlarmConditionResponse>)
  ...
  def "Get alarm condition by Id - Not found and Bad request"() {
    when:
    def response
            = restClient.get("${ALARM_CONDITION_PATH}/12", ApiResponse<RuntimeException>)
  ...
  def "Get alarm condition by Id - Invalid format input - Bad request"() {
    when:
    def response
            = restClient.get("${ALARM_CONDITION_PATH}/abc",
