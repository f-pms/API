...
class AlarmConditionControllerQueryEndpointsFunctionalSpec extends FunctionalTestSpec {
  ...
  def "Get all alarm conditions - OK"() {
    when:
    def response
            = restClient.get("${ALARM_CONDITION_PATH}", dataFixture.ADMIN_USER, List<AlarmConditionResponse>)

    then:
    response.statusCode.is2xxSuccessful()
    List<AlarmConditionResponse> listConditions = response.body.data as List<AlarmConditionResponse>
    listConditions.size() == 1
    listConditions.first().id == DEFAULT_CONDITION_ID
  }

  def "Get alarm condition by Id - OK"() {
    when:
    def response
            = restClient.get("${ALARM_CONDITION_PATH}/$DEFAULT_CONDITION_ID", dataFixture.ADMIN_USER, AlarmConditionResponse)

    then:
    response.statusCode.is2xxSuccessful()
    response.body.data.id == DEFAULT_CONDITION_ID
  }

  def "Get alarm condition by Id - Not found and Bad request"() {
    when:
    def response
            = restClient.get("${ALARM_CONDITION_PATH}/12", dataFixture.ADMIN_USER, RuntimeException)
